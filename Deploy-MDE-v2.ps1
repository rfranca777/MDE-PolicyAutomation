...
function Safe-Parse {
    # PS 5.1 ISE safe JSON parsing. az CLI returns string[] in PS 5.1 ISE,
    # which breaks pipeline | ConvertFrom-Json. This joins to single string first.
    param([object]$RawOutput)
    if ($null -eq $RawOutput) { return $null }
    try {
        $joined = ($RawOutput | Out-String).Trim()
...
$currentContext = Safe-Parse $currentContextRaw
...
$subsAll = Safe-Parse $subsRaw
if (-not $subsAll) { $subsAll = @() }
...
$existingRg = Safe-Parse $existingRgRaw
if ($existingRg) {
...
$vmsParsed = Safe-Parse $vmsRaw
$vms = @()
if ($vmsParsed) { $vms = @($vmsParsed) }
...
if (-not $appId -or -not $appObjectId -or $matched.Count -eq 0) {
    ...
    $secretResult = Safe-Parse $secretResultRaw
    $clientSecret = $null
